...
class ModelOpenbayEbayTemplate extends Model {
	...
	public function get($id) {
		...
		if ($qry->num_rows) {
			$row = $qry->row;
			$row['link_edit'] = $this->url->ssl('openbay/ebay_template/edit&token=' . $this->session->data['token'] . '&template_id=' . $row['template_id'], true);
			$row['link_delete'] = $this->url->ssl('openbay/ebay_template/delete&token=' . $this->session->data['token'] . '&template_id=' . $row['template_id'], true);
