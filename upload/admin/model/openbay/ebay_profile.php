...
class ModelOpenbayEbayProfile extends Model{
	...
	public function get($id) {
		...
		if ($qry->num_rows) {
			$row                = $qry->row;
			$row['link_edit']   = $this->url->ssl('openbay/ebay_profile/edit', 'token=' . $this->session->data['token'] . '&ebay_profile_id=' . $row['ebay_profile_id'], true);
			$row['link_delete'] = $this->url->ssl('openbay/ebay_profile/delete', 'token=' . $this->session->data['token'] . '&ebay_profile_id=' . $row['ebay_profile_id'], true);
