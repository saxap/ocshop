...
class ControllerAccountReturn extends Controller {
	...
	public function index() {
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->ssl('account/return', '', true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->ssl('account/return', $url, true)
		);
		...
		foreach ($results as $result) {
			$data['returns'][] = array(
				...
				'href'       => $this->url->ssl('account/return/info', 'return_id=' . $result['return_id'] . $url, true)
			);
		}
	...
	public function info() {
		...
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->ssl('account/return/info', 'return_id=' . $return_id, true);
		...
		if ($return_info) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_home'),
				'href' => $this->url->ssl('common/home', '', true)
			);
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->ssl('account/return/info', 'return_id=' . $this->request->get['return_id'] . $url, true)
			);
			...
			$data['continue'] = $this->url->ssl('account/return', $url, true);
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->ssl('account/return', '', true)
			);
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('text_return'),
				'href' => $this->url->ssl('account/return/info', 'return_id=' . $return_id . $url, true)
			);
			...
			$data['continue'] = $this->url->ssl('account/return', '', true);
	...
	public function add() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->ssl('account/return/add', '', true)
		);
		...
		$data['action'] = $this->url->ssl('account/return/add', '', true);
		...
		if ($this->config->get('config_return_id')) {
			...
			if ($information_info) {
				$data['text_agree'] = sprintf($this->language->get('text_agree'), $this->url->ssl('information/information/agree', 'information_id=' . $this->config->get('config_return_id'), true), $information_info['title'], $information_info['title']);
			} else {
				$data['text_agree'] = '';
			}
		} else {
