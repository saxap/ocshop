...
class ControllerPaymentGlobalpay extends Controller {
	...
	public function notify() {
		...
		if ($hash != $this->request->post['SHA1HASH']) {
			...
			$data['text_link'] = sprintf($this->language->get('text_link'), $this->url->ssl('checkout/checkout', '', true));
		} else {
			$this->load->model('checkout/order');
			...
			if ($this->request->post['RESULT'] == "00") {
				...
				$data['text_link'] = sprintf($this->language->get('text_link'), $this->url->ssl('checkout/success', '', true));
