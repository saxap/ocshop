...
class ControllerPaymentAmazonLoginPay extends Controller {
	public function address() {
		...
		if (!$this->customer->isLogged() || !isset($this->request->cookie['amazon_Login_state_cache'])) {
			...
			$this->response->redirect($this->url->ssl('payment/amazon_login_pay/failure', '', true));
		}
		...
		$data['amazon_payment'] = $this->url->ssl('payment/amazon_login_pay/orderreference', '', true);
		$data['order_reference'] = $this->url->ssl('payment/amazon_login_pay/shippingquotes', '', true);
		$data['shipping_quotes'] = $this->url->ssl('payment/amazon_login_pay/shippingquotes', '', true);
		$data['payment_method'] = $this->url->ssl('payment/amazon_login_pay/paymentmethod', '', true);
	...
	public function paymentMethod() {
		...
		$data['confirm_order'] = $this->url->ssl('payment/amazon_login_pay/confirm', '', true);

		$amazon_payment_js = $this->model_payment_amazon_login_pay->getWidgetJs();
		$this->document->addScript($amazon_payment_js);

		$data['continue'] = $this->url->ssl('payment/amazon_login_pay/confirm', '', true);
		$data['back'] = $this->url->ssl('payment/amazon_login_pay/address', '', true);
		$data['text_back'] = $this->language->get('text_back');
	...
	public function confirm() {
		...
		if (!$this->customer->isLogged() || !isset($this->request->cookie['amazon_Login_state_cache'])) {
			$this->session->data['lpa']['error'] = $this->language->get('error_login');
			$this->response->redirect($this->url->ssl('payment/amazon_login_pay/loginFailure', '', true));
		}
		...
		if (!isset($this->session->data['lpa']['shipping_method']) || !isset($this->session->data['lpa']['address'])) {
			$this->response->redirect($this->url->ssl('payment/amazon_login_pay/address', '', true));
		}
		...
		$data['merchant_id'] = $this->config->get('amazon_login_pay_merchant_id');
		$data['process_order'] = $this->url->ssl('payment/amazon_login_pay/processorder', '', true);
		...
		$data['back'] = $this->url->ssl('payment/amazon_login_pay/paymentMethod', '', true);
		$data['text_back'] = $this->language->get('text_back');
	...
	public function processOrder() {
		...
		if (isset($response['redirect'])) {
			...
			$this->response->redirect($this->url->ssl('payment/amazon_login_pay/' . $response['redirect'], '', true));
		}
		...
		if ($response['status'] == 'Closed' || $response['status'] == 'Open') {
			...
			$this->response->redirect($this->url->ssl('payment/amazon_login_pay/success', '', true));
		} else {
			$this->failure($this->language->get('error_process_order'));
		}
	}
	...
	public function failure($error) {
		...
		$this->response->redirect($this->url->ssl('checkout/cart', '', true));
	}
	...
	public function setShipping() {
		...
		if (isset($this->request->post['shipping_method'])) {
			...
			$json['redirect'] = $this->url->ssl('payment/amazon_login_pay/paymentMethod', '', true);
		} else {
			$json['redirect'] = $this->url->ssl('payment/amazon_login_pay/paymentMethod', '', true);
		}
